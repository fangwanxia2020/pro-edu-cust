...
class C {}
// 初始化方法
C.prototype.initilize = function (opt) {
  // console.log(opt)
  this.options = opt
  // 创建dom
  this.createElement()
  ...
  this.cropper = new Cropper(this.preview, {
    aspectRatio: opt.aspectRatio ? opt.aspectRatio : NaN,
    // aspectRatio: 4/3,
    viewMode: 1,
    background: true,
    zoomable: true,
...
C.prototype.createElement = function () {
  ...
  str +=
    '<div class="crop_loading"><div class="crop_content"><div class="crop_text">图片上传中</div></div></div>'
...
// 初始化一些函数绑定
C.prototype.initFunction = function () {
  let self = this
...
C.prototype.clip = function (e, opt) {
  // console.log(e)
  let self = this
...
// 图片上传
C.prototype.postImg = function (imgData) {
  // 这边写图片的上传
  // blob
  let file = convertBase64UrlToFile(imgData, '123.png')
  // console.log(file)
  // 常量
  const $ = (el) => document.querySelector(el)
  const $l = $('.crop_loading').style
  const $s = $('.crop_success').style
  const remove = () => {
    setTimeout(() => {
      this.destoried()
    }, 500)
...
// 图片旋转
C.prototype.rotateImg = function (img, direction, canvas, _step) {
  // 180度有bug 增加_step参数
