...
<template>
  <div class="org-con">
    ...
    >
      <div slot="table_oper" class="searchBox">
        <ul>
          <li>
            <label>组织名称：</label>
            <el-input
          ...
          <li>
            <el-button icon="el-icon-refresh" size="mini" @click="clearFilter">重置</el-button>
          </li>
        </ul>
        <ul>
          <li>
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="showInfo('new')"
              >新增</el-button
            >
          </li>
          <li>
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showInfo('modify')"
              >修改</el-button
            >
          </li>
          <li>
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteItem"
              >删除</el-button
            >
          </li>
        </ul>
    ...
    <ty-dialog
      ...
      :onOk="submitInfo"
    >
      <pc-form :data="orgJson" v-model="orgData" ref="orgForm" v-if="orgData">
        <template slot="addressSlot">
          <ty-select-area v-model="orgData.region" width="300px"></ty-select-area>
          <el-input
...
<script>
import { getOrgList, removeOrg, modifyOrg, addOrg } from '@/api/basicData/org'
import { getOrgJson } from './orgForm'
  ...
  methods: {
    getList() {
      ...
      if (this.searchObj.timeRange) {
        sendData.createTimeStart = this.toDateUtil.formatDateTime(
          this.searchObj.timeRange[0],
          'yyyy-MM-dd HH:mm:ss'
        )
        sendData.createTimeEnd = this.toDateUtil.formatDateTime(
          this.searchObj.timeRange[1],
          'yyyy-MM-dd HH:mm:ss'
        )
      }
