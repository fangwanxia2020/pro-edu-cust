        ...
        >
          ...
          <template #personNum="{ data }">
            <el-link
              type="primary"
              @click="
                navigater('/personBasicInfo/viewPersonBasicInfo', {
                  enterprisePersonId: data.enterprisePersonId
                })
              "
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="closeAccountDialog(true)">确 定</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
import { getAccountDialogFormData, getColumns } from './jsonData'
...
export default {
  ...
  computed: {
    ...
    accountDialogFormData() {
      const { accountDialog } = this
      const { editData } = accountDialog
      return getAccountDialogFormData(editData.deptId, accountDialog.title === '账号详情')
    }
  },
  watch: {
    // 根据名称筛选部门树
    'deptTree.deptName': function (val) {
  ...
  methods: {
    ...
    handleNodeClick(data) {
      const { personBasicInfoCrud } = this.$refs
