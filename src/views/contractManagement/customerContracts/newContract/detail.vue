<template>
  ...
  >
    ...
    <el-dialog
      ...
      top="20px"
    >
      <el-form :model="supplierParams" ref="queryForm" label-width="100px">
        <el-row>
          <el-col :span="8">
...
import {
  ...
  editCustomer,
  getInfo
} from '@/api/contractManagement/customer'
import { editCooperativePurposeStatus } from '@/api/cooperation/intention'
...
export default {
  ...
  watch: {
    'addData.connectStatus': function (value) {
      console.log('addData.connectStatus', value, this.CompanyList)
      if (value === '否') {
        this.CompanyList = []
  ...
  methods: {
    getPartner() {
      ...
      getInfo().then((res) => {
        if (res.code === 200) {
          console.log(1234)
          this.$set(this.Partner, 'supplierName', res.user.org.orgName)
          this.$set(this.Partner, 'contacts', res.user.org.username)
          this.$set(this.Partner, 'phone', res.user.org.phone)
          this.$set(this.Partner, 'supplierId', res.user.org.id)
          this.$set(this.Partner, 'detailAddress', res.user.org.address)
    ...
    save() {
      this.$refs.supplierFrom.getData().then((res) => {
        ...
        if (this.id !== '') {
          ...
          addCustomer(obj).then(async (ress) => {
            if (ress.code === 200) {
              if (this.$route.query.name === 'intention') {
                let params = {
                  cooperativePurposeId: this.$route.query.cooperativePurposeId,
                  status: 3
                }
                let rest = await editCooperativePurposeStatus(params)
              }
              if (this.$route.query.name === 'needMatch') {
                let params = {
                  recommendedEnterpriseRelId: this.$route.query.recommendedEnterpriseRelId
                  // status: 3
                }
                // let rest = await editRecommendedEnterpriseRelStatus(params)
              }
              this.$message.success('保存成功')
              this.id = ress.data
    ...
    selectionChange2(e) {
      ...
      this.selectionRows2 = e
      this.supplierRows = []
      this.supplierRows = this.selectionRows2.concat(this.selectionRows3)
    ...
    switchChange(row) {
      console.log(' switchChange this.CompanyList', this.CompanyList)
    ...
    ok() {
      ...
      if (JSON.stringify(this.tableselectionRows) == '{}') {
        this.dialogVisible = false
        return
      }
      ...
      this.addData.partyACompanyOrgId = this.tableselectionRows.relevanceOrgId
      this.addData.partyAorgId = this.tableselectionRows.relevanceOrgId
      this.page1.pageNo = 1
      this.projectManagerParams.templateId = ''
      this.projectManagerParams.customerName = ''
      this.projectManagerParams.detailAddress = ''
    ...
    supplierOk() {
      ...
      this.supplierRows.forEach((item) => {
        ...
        obj.partyBOrgId = item.createOrgId
        if (item.supplierId) {
    ...
    getcustomerList() {
      ...
      obj.pageNum = this.page1.pageNo
      obj.pageSize = this.page1.limit
      // obj.bindingStatus = 2
      // obj.isAdd = 1
    ...
    getcustomerSupplier() {
      ...
      obj.customerName = this.supplierParams.supplierName
      obj.contacts = this.supplierParams.contacts
      obj.phone = this.supplierParams.phone
      ...
      customerPartner(obj).then((res) => {
        ...
        this.customerSupplier = res.data.records
        console.log('123456', this.customerSupplier)
    ...
    async getSupplierList() {
      ...
      await supplierPartner(obj).then((res) => {
        this.supplierList = []
    ...
    async resetQuery2() {
      ...
      this.supplierList.unshift(this.Partner)
      console.log('this.supplierList.unshift(this.Partner)', this.supplierList)
      this.page3.total += 1
      this.getcustomerSupplier()
    },
    resetQuery() {
