          ...
          <el-timeline>
            ...
            >
              <el-card>
                ...
                <div>提醒谁看: {{ item.remindList || '无' }}</div>
...
import {
  getRecord,
  deletePeFollowRemind,
  getDetails,
  getAbility
} from '@/api/cooperation/record.js'
import getColumns from './abilityRecord.js'
...
export default {
  ...
  created() {
    ...
    this.getTimeLine()
    this.getAbilityData()
    console.log(this.$route.query.recommendedEnterpriseRelId, 'lihdoisahd')
  },
  mounted() {},
  computed: {
    timestamp() {
  ...
  methods: {
    editDetail(item) {
      getDetails(item.followRemindId).then((res) => {
        console.log(item.followRemindId)
    ...
    getAbilityData() {
      getAbility(this.recommendedEnterpriseRelId).then((res) => {
