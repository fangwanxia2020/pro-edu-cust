  ...
  <div>
    ...
    <div class="serachModule">
      <el-form :model="queryParams" ref="queryForm" label-width="100px">
        <el-row>
          <el-col :span="8">
            <el-form-item label="我的供给" prop="supply">
              <el-input
          ...
          <el-col :span="8">
            <el-form-item label="对方需求" prop="demand">
              <el-input
        ...
        <div v-show="isShow">
          <el-row>
            <el-col :span="8">
              <el-form-item label="我的能力" prop="ability">
                <el-input
...
} from '@/api/cooperation/abilityMatch'
import { autoRecommendedEnterprise } from '@/api/cooperation/needMatch'
...
export default {
  ...
  methods: {
    async init() {
      ...
      console.log('this.listTypeArr', this.listTypeArr)
    ...
    async resetList() {
      // const { orgId } = this.$store.state.user.userInfo
      // const res = await abilityCount(orgId)
      // if (res.data) {
    ...
    getList() {
      ...
      abilityMatchingList(sendData).then((response) => {
    ...
    // 跟进记录
    followRecord(row) {
      console.log(row.recommendedEnterpriseRelId, 'kan')
      this.$router.push({
