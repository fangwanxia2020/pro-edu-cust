          ...
          <el-timeline>
            ...
            >
              <el-card>
                ...
                <div>
                  跟进提醒:
                  {{ item.remindType | filtersRemindType(item.remindTimeValue) }}
                </div>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      getList(this.cooperativePurposeId).then((res) => {
    ...
    getTimeLine() {
      getRecord({
        cooperationDemandId: this.cooperativePurposeId,
        source: this.source
      }).then((res) => {
