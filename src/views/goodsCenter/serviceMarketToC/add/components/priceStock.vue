<template>
  <div id="price-stock">
    ...
    >
      <div class="sale-type">
        <el-form-item label="销售单位" class="width525">
          <el-select
            v-model="unitId"
            placeholder="请选择销售单位"
            :disabled="isDetail"
        ...
        <el-form-item label="单品/多规格" class="width525">
          <el-radio-group
            v-model="ruleForm.skuType"
            @change="changeSkuType"
            :disabled="isModify || isDetail"
          >
        ...
        <el-form-item label="商品规格" v-if="ruleForm.skuType == 0">
          ...
          >
            ...
            <div class="clearfix">
              ...
              <div class="input-item">
                <el-button type="text" @click="addSpec(index)" :disabled="isModify || isDetail"
                  >添加规格值</el-button
                >
              </div>
            </div>
          ...
          <div>
            <el-button type="primary" @click="addSpecItem" :disabled="isModify || isDetail"
              >添加规格项目</el-button
            >
    ...
    <div class="time-btn" v-if="!isDetail">
      ...
      <el-button type="primary" @click="submit(2)">保存并进行下一步</el-button>
    </div>
...
export default {
  data() {
    return {
      ...
      modeList: [], // 账目类型
      unitTxt: '',
      shopShelvesId: SHOP_SHELVES_ID.TOC, // 货架ID
  ...
  methods: {
    ...
    async submit(type) {
      ...
      for (var i in this.prodAttrValue2) {
      ...
      let skuObj = {
        ...
        goodsSku,
        isThirdSku: this.ruleForm.skuType,
        prodAttrStock,
        reduceStockType: 1,
      ...
      modifyStock(skuObj).then((res) => {
        if (res.code === 200) {
          ...
          if (type == 1 || type == 2) this.getStockInfo(goodsId, this.shopShelvesId)
          if (type == 1) {
    ...
    async getStockInfo(goodsId, shopShelvesId) {
      ...
      if (data.prodAttrStock.length > 0) {
        ...
        this.unitId = data.unitId ? `${data.unitId}` : ''
        // 赋值规格名
        let specList = []
    ...
    specShow1(datas) {
      ...
      datas.forEach((item) => {
        ...
        item.stockNum = item.stockNumList.length > 0 ? item.stockNumList[0].stockNum : item.stockNum
        for (let j in list) {
        ...
        for (let j in item.goodsSkuPrice) {
          ...
          item[item1.saleAccountTypeId] = item1.price
        }
    ...
    composeSku() {
      ...
      for (let i in tableData) {
        ...
        let goodsSkuPrice = []
        for (let j of this.modeList) {
          if (item[j[SALE_ACCOUNT_TYPE_ID]] == '' || item[j[SALE_ACCOUNT_TYPE_ID]] == null) {
          ...
          goodsSkuPrice.push({
            price: item[j[SALE_ACCOUNT_TYPE_ID]],
    ...
    catalogImg(file) {
      ...
      //       let index = this.uploadImgSpecIndex
      //       this.ruleForm.tableData[index].skuImg = data.responseObject[0].fileUrl
      //     } else {
      //       this.$message({
      //         showClose: true,
