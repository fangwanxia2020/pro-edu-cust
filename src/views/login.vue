<template>
  <div v-show="showPage" class="login-container">
    <!-- admin -->
    <img :src="bgImage" class="bg-img" alt="qiyetupian" v-temp-img="bgImg" />
    <el-form
      ref="loginForm"
      :model="loginForm"
      :rules="loginRules"
    ...
    >
      <div class="title-container">
        ...
        <div class="title">{{ sysName || title }}</div>
      </div>
      ...
      <el-form-item prop="password" class="form-input">
        <el-input
          ...
          @keyup.enter.native="handleLogin"
        >
          <ty-icon slot="prefix" type="password" class="login-password" />
        ...
        <span class="show-pwd" @click="showPwd">
          <i :class="['anticon', passwordType === 'password' ? 'closeEye' : 'openEye']" />
        </span>
      </el-form-item>
      ...
      <el-button
        type="primary"
        style="width: 100%; margin-top: 10%; padding: 10px 20px; font-size: 18px"
        :loading="loading"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      bgImage: require('@/assets/images/login_img/apli.png') //
    }
  },
  watch: {
    $route: {
    ...
    'ruleForm.password': function (val) {
      ...
      let reg1 = new RegExp(/^(?![^a-zA-Z]+$)(?!\D+$)/)
      let reg2 = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^]$/
      let reg3 = /(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[\W])/
      // 密码长度要大于6位，由数字和字母组成
      // var reg1 =/^[0-9]*$
  ...
  methods: {
    async getConfig() {
      ...
      const { name, webName, profilePicture } = store.getters.sysConfig
      if (profilePicture) this.bgImage = profilePicture
      this.sysName = name
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          this.$store
            .dispatch('user/login', RsaLoginForm)
            .then(() => {
              sessionStorage.setItem('enterNum', 1) // 首次进入系统的标志
