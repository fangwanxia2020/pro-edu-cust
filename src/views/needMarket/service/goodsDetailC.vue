            ...
            <div v-else>
              <div class="type">
                ...
                <div :class="{ 'hide-all': !showAll }">
                  ...
                  <div
                    class="label-list"
                    v-for="(item, index) in goodsStockInfoVO.prodAttrStock"
                    :key="index"
                  >
                    <span class="">{{ item.attrName }}:</span>
                    <span
                      :class="['choose-item', item.choose == ite ? 'active' : '']"
                      v-for="ite in item.prodAttrValue"
                      :key="ite"
                      @click="chooseItem(index, ite)"
                      >{{ ite }}</span
                    >
                  </div>
                  <!-- </el-row> -->
                ...
                <div
                  v-if="goodsStockInfoVO.prodAttrStock.length > 3"
                  class="show-all"
...
export default {
    ...
    goodList: {
      handler(val) {
        ...
        this.hasChooseArr.map((ite) => {
          ite.selfTotalPrice = ite.price * ite.amount
  ...
  methods: {
    ...
    // 多规格选择
    chooseItem(index, ite) {
      this.chooseItemInfo = null
      console.log(ite)
      this.$set(this.goodsStockInfoVO.prodAttrStock[index], 'choose', ite)
      console.log(this.goodsStockInfoVO.prodAttrStock[index])
      // 显示选好的价格
      // 颜色:黄,尺寸:1dm
      let titleArr = []
      let hasChooseAll = true
      this.goodsStockInfoVO.prodAttrStock.map((e) => {
        if (!e.choose) {
          hasChooseAll = false
        } else {
          titleArr.push(`${e.attrName}:${e.choose}`)
        }
      })
      if (hasChooseAll) {
        // 找到对应的数组
        this.chooseItemInfo = this.goodsStockInfoVO.goodsSku.find(
          (e) => e.skuAttrValidStr == titleArr.join(',')
        )
        if (this.chooseItemInfo) {
          this.price = this.chooseItemInfo.goodsSkuPrice[0].price
        }
        console.log(titleArr.join(','))
        console.log('this.chooseItemInfo')
        console.log(this.chooseItemInfo)
      }
      // skuAttrValidStr: "颜色:黄,尺寸:1dm"
    },
    toOrder(data) {
      if (!this.toLogin()) {
        return
        ...
        if (res.code === 200) {
          ...
          if (
            ...
            let sigleGoodd = {
              ...
              goodsSkuName: goodsStockInfoVO.goodsSku[0].skuName,
