  ...
  <div>
    <div class="header">
      <el-form inline :model="form">
        ...
        <el-form-item class="formItem">
          <!-- <div class="bbt" style="float: right"> -->
      ...
      <div class="tab">
        <table border="1">
          ...
          <tbody v-for="(item, index) in tabList" :key="index">
            ...
            <tr class="tr-td">
              <td v-for="(i, index) in item.orderItemVos" :key="index">
                <div class="img">
              ...
              <td>
                <div>
                  <!-- <el-button type="text" @click="toPay(item.orderId)" v-if="item.orderStatus == 0">
                    去付款
                  </el-button> -->
                  <el-button
                    type="text"
    ...
    <div class="noteDialog">
      <el-dialog title="备注" :visible.sync="dialogVisible" width="30%">
        ...
        <div style="margin-top: 10px">商品名称：{{ goodsName }}</div>
        <div style="display: flex; margin-top: 10px">
          <div style="width: 50px">备注：</div>
          <el-input v-model="noteInput" type="textarea"></el-input>
        </div>

        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisible = false">取 消</el-button>
          <el-button type="primary" @click="noteSumbit()">确 定</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
...
import { listPort, cancelOrderPort, editOrder } from '@/api/orderCenter/order.js'
...
export default {
  ...
  methods: {
    ...
    note(row) {
      console.log(row, 'row')
      if (row.orderItemVos.length != 0) {
        this.goodsName = row.orderItemVos[0].goodsName
      } else this.goodsName = '-'
    ...
    noteSumbit() {
      editOrder({ orderId: this.id, businessNote: this.noteInput }).then((res) => {
