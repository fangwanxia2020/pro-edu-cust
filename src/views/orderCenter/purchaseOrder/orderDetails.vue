<template>
  <div class="body">
    ...
    <div class="reserve-form">
      <div class="header-title">下单人信息</div>
      <div class="product-info all-info">
        <div style="margin-bottom: 10px">
...
<script>
import { listDetail, listNumDetail } from '@/api/orderCenter/order.js'
...
export default {
  ...
  data() {
    return {
      ...
      orderNum: Number(this.$route.query.orderNum),
      total: null,
  ...
  methods: {
    ...
    getData() {
      if (this.orderNum) {
        listNumDetail(this.orderNum).then((res) => {
          this.orderInfo = res.data
          if (!this.orderInfo.orderId) {
            this.$message.warning('没有查到对应的订单信息')
            return
          }
        ...
        listDetail(this.orderId).then((res) => {
          this.orderInfo = res.data
          res.data.orderItemVos.map((e) => {
            this.total += e.orderItemFee
          })
