...
import { checkAddAddressBook } from '@/api/partnerManagement/partner'
...
export default {
  ...
  data() {
    return {
      columns: null,
      getListInterface: {
        url: '/system/customerPartner/listsOfApplySupplierWithPage',
        methodType: 'get',
        params: {
          pageSize: 10,
          pageNum: 1
  ...
  methods: {
    ...
    addContacts(row) {
      this.confirm('确认添加到通讯录？', () => {
        checkAddAddressBook(row.supplierId).then((res) => {
          this.selectRows = []
    ...
    //     })
    //     this.$router.push({
    //       path: `/partner/approveClient/${EDIT}/${templateId}`,
    //       query: {
    ...
    async approvedetail(row, flag) {
      ...
      if (res.code === 200) {
        ...
        console.log(templateId)
        this.$router.push({
          path: `/partner/approveClient/${DETAIL}/${templateId}`,
          query: {
            projectId: row.projectId,
            flag,
            supplierId: row.supplierId,
