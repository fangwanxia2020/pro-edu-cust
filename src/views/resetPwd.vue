...
<template>
  <div class="container-reset-pwd">
    <div class="main-content">
      <p class="title"><span></span> {{ $route.query.type ? '找回密码' : '修改密码' }}</p>
      <div class="form-box">
        <el-form
          :model="ruleForm"
          status-icon
        ...
        >
          ...
          <el-form-item label="密码强度：" class="mfqd" v-show="ruleForm.password">
...
<script>
import { sendResetPhone, resetPassword } from '@/api/user'
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const params = {
            ...
            phone: mobile,
            tenantCode: 'admin' // sessionStorage.getItem('roleKey')
          }

          resetPassword(params).then(async (data) => {
            if (data.code != 200) return
            const { tenantCode } = this.$route.query
            // 清空商户信息
            if (!tenantCode) {
              localStorage.setItem('navLogo', '')
              localStorage.setItem('navMenuName', '')
              const token = getToken()
              if (token) await this.$store.dispatch('user/logout')
            }
            // const zuhu = tenantCode || params.tenantCode
            // this.$router.push({
            //   path: `/login/${zuhu}`
            // })
            this.$router.push({
