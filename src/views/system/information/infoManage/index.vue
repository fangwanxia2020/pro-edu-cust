<template>
  <div class="container">
    <div v-if="columns.length" class="content">
      ...
      <ty-business-crud
        ...
        :columns="columns"
        :border="false"
        :showSearchFields="['title', 'publishName', 'publishStatus', 'publishWay']"
        ref="materialCrud"
      ...
      >
        <div slot="title" slot-scope="{ data }">
          <el-tooltip class="item" effect="light" :content="data.title" placement="top">
            <el-link type="primary">
              <router-link
                :to="{ path: '/information/infoManage/newsContent', query: { msgId: data.msgId } }"
              >
                <span class="tab_title">{{ data.title }}</span>
              </router-link>
            </el-link>
          </el-tooltip>
        </div>
        <template #btnsBar>
          <div class="btns-bar">
            <el-button type="primary" size="small" @click="handleEditFun('add')"
              >创建消息</el-button
            >
          </div>
        </template>
        ...
        <template v-slot:rowOperator="{ data }">
          <el-button
            ...
            >查看</el-button
          >
          <el-button type="text" v-if="data.publishStatus === 0" @click="handleEditFun('add', data)"
            >编辑</el-button
          >
          <el-button type="text" v-if="data.publishStatus === 0" @click="deleteMsg(data)"
            >删除</el-button
          >
        </template>
      </ty-business-crud>
...
import { removeByIds } from '@/api/system/information'
...
export default {
  ...
  methods: {
    ...
    resetSearchFun() {
      ...
      this.searchDateRang = []
      this.getListInterface.params = { isTpl: 0 }
    }
  }
}
</script>
