  ...
  <div class="material-edit design-form-wrap">
    ...
    <div class="design-form-wrap__btnwrap">
      <el-button
        ...
        >保存</el-button
      >
      <el-button size="small" type="primary" @click="navigater(`/emergencyManage/drillPlan`)"
        >发布</el-button
      >
      <el-button size="small" @click="navigater(`/emergencyManage/drillPlan`)">{{
        this.getPathRoute('select') ? '返回' : '取消'
      }}</el-button>

      <el-button size="small" @click="navigater(`/emergencyManage/drillPlan`)">重置</el-button>
    </div>
  </div>
</template>
...
import getJsonData from './js/editNews.js'
...
export default {
  ...
  created() {
    ...
    this.jsonData = getJsonData({ type, title })
  ...
  methods: {
    ...
    selectPersonOk(selectedData) {
      ...
      const { nickName, userId, deptId, deptName } = selectedData[0]
      const { editData } = this
