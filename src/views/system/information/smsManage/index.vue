<template>
  <div class="container">
    <div v-if="columns.length" class="content">
      ...
      >
        <div slot="checkTitle" slot-scope="{ data }">
          <el-link type="primary">
            <router-link
              :to="{ path: '/information/insideInfoManage/editNews', query: { type: 1 } }"
              >{{ data.checkTitle }}</router-link
            >
          </el-link>
        </div>
        <div slot="checkNso" slot-scope="{}">
          <el-link type="primary">
            <router-link
              :to="{ path: '/information/insideInfoManage/editNews', query: { type: 2 } }"
              >查看人员</router-link
            >
          </el-link>
        </div>
        ...
        <template v-slot:rowOperator="scope">
          <el-button type="text" @click="handleEditFun('newsContent', scope.row)">启用</el-button>
        </template>
      </ty-business-crud>
    </div>
  </div>
